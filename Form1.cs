    ...
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            fileSystemWatcher1.Renamed += async (s, eventArgs) =>
            {
                ...
                if (cacheRelation.RemoteLastWriteTime != File.GetLastWriteTime(cacheRelation.RemotePath))
                {
                ...
                try
                {
                    ...
                    try
                    {
                        File.WriteAllText(Path.Combine(Environment.GetFolderPath(
                            Environment.SpecialFolder.LocalApplicationData), @"PowerCacheOffice\cacheSettings.json"), JsonSerializer.Serialize(cacheSettings));
                    }
                    catch { }
                }
                catch (Exception ex) { MessageBox.Show(ex.Message, Program.AppName); }
            ...
            listBox1.KeyDown += (s, eventArgs) =>
            {
                if (eventArgs.KeyData == Keys.Delete)
                {
                    ...
                    try
                    {
                        File.WriteAllText(Path.Combine(Application.StartupPath, "appSettings.json"), JsonSerializer.Serialize(appSettings));
                    }
                    catch { }
                }
            };
            ...
            foreach (var filePath in args)
            {
                ...
                if (listBox1.Items.OfType<string>().Any(x => filePath.ToLower().StartsWith(x.ToLower())))
                {
                    ...
                    else
                    {
                        ...
                        if (cacheRelation.RemoteLastWriteTime != lastWriteTime)
                        {
